...
const getPageImageUrl = (document) => {
    ...
    return el && el.style.backgroundImage ? el.style.backgroundImage.replace(/^(url\()*("|')|("|')(\))*(;)*$/g, '') : null;
}
...
const getPageDate = (document) => {
    ...
    return el ? moment(el.textContent.replace(/[^0-9.]/g, ''), 'DD.MM.YY').format() : null;
}

const renderFavoritesButton = ({type, active, onPress}, document) => {
    const buttonWrapper = document.getElementById('nts-favorites-extension') || document.createElement('div');
    ...
    buttonWrapper.innerHTML = `
        <button type="button" class="nts-app nts-link nts-link--black text-uppercase text-bold text-right" style="padding: 12px">
            <svg xmlns="http://www.w3.org/2000/svg" style="position: relative; top: -1px; margin-right: 2px" viewBox="0 0 34 30" height="14" fill="#000">
                ${active && '<path d="M17 29.7l-.6-.5C3.5 18.7 0 15 0 9c0-5 4-9 9-9 4.1 0 6.4 2.3 8 4.1C18.6 2.3 20.9 0 25 0c5 0 9 4 9 9 0 6-3.5 9.7-16.4 20.2l-.6.5z"/>'}
