        ...
        <div class="row" style="margin-top:1rem;margin-bottom:2rem">
            ...
            <div class="col-12 col-md-6" style="display: flex; justify-content: flex-end">
                <div class="search-input">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 600" height="24">
                        <path fill="#fff"
                              d="M413.38 392.4a169.89 169.89 0 0 0 18.19-27.4c43-83 10.73-185.28-72.09-228.39a168.63 168.63 0 0 0-227.82 72.27c-43 83-10.74 185.28 72.09 228.39a168.49 168.49 0 0 0 183.35-18L461.62 494l26.56-26.62zm-194.25 15.16C152.67 373 126.77 290.9 161.28 224.28a135.43 135.43 0 0 1 182.82-58c66.46 34.6 92.36 116.66 57.85 183.28a135.43 135.43 0 0 1-182.82 58z"></path>
                    </svg>
                    <input type="search" v-model="searchTerm" autofocus v-debounce="filterFavorites">
                </div>
            </div>
            <div class="col-12" style="margin-top:1rem" v-if="searchTerm">
                <p>There {{ favorites.length == 1 ? 'is' : 'are' }} <strong>{{ favorites.length }}</strong>
                    result{{ favorites.length == 1 ? '' : 's' }} for "{{ searchTerm }}"</p>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            filterFavorites() {
                this.favorites = this.searchTerm ? this.indexedFavorites.search(this.searchTerm) : this.indexedFavorites.list
            }
        },
        computed: {
            episodes() {
                return this.favorites.map(e => e.item ? e.item : e).filter(item => item.type == EPISODE)
            },
            shows() {
                return this.favorites.map(e => e.item ? e.item : e).filter(item => item.type == SHOW)
            }
        },
        filters: {
            date(value) {
